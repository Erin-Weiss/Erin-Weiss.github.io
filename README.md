...
Turns a client's demolition site list into a complete neighbor notification package, replacing what the client estimated as days of parcel-by-parcel lookup with a run that finishes in about a minute. Searches all **126,958 St. Louis parcels** against 500 ft buffers measured from each site's boundary rather than its center, identifying **1,636 notification addresses** across a 30-site project and flagging **134 parcels** whose assessor records contradict themselves for field verification. Delivers door hanger lists, per-site walking checklists ordered the way crews walk, an auditable assumptions log, and an interactive map that works with no internet connection and publishes to a live link with a scannable code printed onto the checklists. Ships as both a Streamlit web app and a command-line tool running the identical pipeline, validated by over 100 tests, ruff linting, and a CI check that re-derives every delivered headline figure on each push.

**Tech:** `Python` · `GeoPandas` · `Shapely` · `Folium` · `Leaflet.js` · `Streamlit` · `pandas` · `openpyxl` · `GeoParquet` · `Netlify` · `segno` · `pytest` · `ruff` · `GitHub Actions`  
**Links:** [Live Report](https://erin-weiss.github.io/stl-demo-notify/) · [Live App](https://stl-demo-notify.streamlit.app/) · [Published Map](https://stl-demo-notify-2026-08-06.netlify.app) · [GitHub Repo](https://github.com/Erin-Weiss/stl-demo-notify) · [Portfolio Page](https://erin-weiss.github.io/articles/STL-Demo-Notify.html)
...
**MLOps & Deployment**  
`FastAPI` · `Docker` · `Kubernetes` · `GitHub Actions (CI/CD)` · `Prometheus` · `Grafana` · `Pydantic` · `Uvicorn` · `pytest` · `ruff` · `Streamlit Cloud` · `Netlify` · `GHCR`
